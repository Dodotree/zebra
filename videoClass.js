        ...
        navigator.mediaDevices
            ...
            .then(stream => {
                ...
                stream.getTracks().forEach(track => {
                    ...
                    // TO DO 1: standard resolutions for RealSense, Razor and render similar variants for unknown
...
const utilsUI = {
    ...
    getInputUI(cKey, cOptions, cValue, callback) {
        ...
        if (typeof cOptions === "string" || cOptions instanceof String) { // string or String wrapper
            ...
            pElement.appendChild(utilsUI.get({
                tag: "label",
                text: cKey,
                attrs: { htmlFor: cKey }
            }));
            const inp = pElement.appendChild(utilsUI.get({
                tag: "input",
                attrs: {
                    type: "text",
                    name: cKey,
            ...
            const sel = pElement.appendChild(utilsUI.get({
                tag: "select",
            ...
            const inpRange = pElement.appendChild(utilsUI.get({
                tag: "input",
                attrs: {
                    type: "range",
                    name: cKey + "Range",
            ...
            const inpNum = pElement.appendChild(utilsUI.get({
                tag: "input",
                attrs: {
                    type: "number",
                    name: cKey + "Number",
